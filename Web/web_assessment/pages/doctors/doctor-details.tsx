...
const DoctorDetailPage = () => {
  ...
  if (isLoading) {
    ...
    return (
      <div>
        Error doctorLength:{doctor} error:{error ? error.toString() : ""}
      </div>
    );
  }
  return (
      ...
      <div className="flex flex-row justify-between w-full gap-y-2">
        <div className="flex flex-col">
          ...
          <div className="text-navbar-text">
            {doctor.specialty && doctor.speciality.length
              ? doctor.specialty[0].name
              : "Cardiologist"}
          </div>
        </div>
        <div className="flex flex-col">
          <p className="text-navbar-text text-sm">
            {doctor.education && doctor.education.length
              ? doctor.education[0].name
