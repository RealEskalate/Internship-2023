...
const TeamMemberCard: React.FC<TeamMemberCardProps> = ({teamMember}) => {
  return (
    <div className="flex flex-col self-start bg-white rounded-lg p-6 m-2 items-center justify-center shadow-xl max-w-[500px]">
      ...
      <h1 className="font-bold uppercase text-black text-2xl m-3">{teamMember.name}</h1>
      <h2 className="uppercase text-black text-xl">{teamMember.job}</h2>
      <p className="text-center my-4 text-xl text-[#7D7D7D]">{teamMember.description}</p>
