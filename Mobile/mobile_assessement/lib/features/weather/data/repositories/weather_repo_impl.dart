...
class WeatherRepositoryImpl implements WeatherRepository {
  ...
  @override
  Future<Either<Failure, Weather>> GetFavCity() async {
    final networkStatus = await networkInfo.isConnected;
    try {
      if (!networkStatus){
        return Left(ServerFailure());
      }
      final weather = await weatherRemoteDataSource.getFavouriteCity();
      return Right(List<Weather>);
    } on ServerException {
      return Left(ServerFailure());
    }
  }
  ...
  @override
  Future<Either<Failure, Weather>> searchCity(String city) async {
    final networkStatus = await networkInfo.isConnected;
    try {
      if (!networkStatus){
        return Left(ServerFailure());
      }
      final weather = await weatherRemoteDataSource.searchCity(city);
      return Right(weather);
    } on ServerException {
      return Left(ServerFailure());
    }
